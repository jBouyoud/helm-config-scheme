...
:: If HELM_CONFIG_SCHEME_WINDOWS_SHELL is provided, use it.
if not "%HELM_CONFIG_SCHEME_WINDOWS_SHELL%"=="" GOTO :ENVSH
...
:ENVSH
IF "%HELM_CONFIG_SCHEME_WINDOWS_SHELL%"=="wsl" GOTO :WSL

"%HELM_CONFIG_SCHEME_WINDOWS_SHELL%" %*
exit /b %errorlevel%
...
echo helm-config-scheme needs a unix shell. Please install WSL, cygwin or Git for Windows.
exit /b %errorlevel% 1
